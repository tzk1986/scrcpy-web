...
- 日期：2026-09-23
- 状态：**已实施并真机复测通过**（修复提交 8305a46；复测 2026-09-23，
  结论见第七节）
...
## 七、真机复测结论（2026-09-23，修复提交 8305a46）

复用方案 19 验证基建（`tests/e2e/specs/stream-stability-19.spec.ts`；
角标判据已由 Playwright 可见性轮询改为页面内 100ms 采样器时间线，
消除 <0.5s 渲染窗漏捕竞态，并加失败取证 afterEach）。后端以
`APP_ENV=e2e` 运行（关闭自适应码率，隔离 O6 降档重启抖动，与 spec
设计一致）。

| 步骤 | 操作 | 结果 |
|---|---|---|
| 1 | .25 600s 静置观察 | ✔ PASS：帧 4→2784（≈4.64fps）、**回退日志 0 条**（修复前 15 轮回退/30 条日志）、最大帧间隔 2.19s、全程 mode=h264/state=直播中、0 角标 0「正在连接」 |
| 2 | .18 600s 静置观察 | ✔ PASS：帧 2→2304（≈3.87fps）、**回退日志 0 条**、最大帧间隔 6.21s（< STALL 10s，静默由保活 RESET 补齐）、全程 h264、0 角标 0「正在连接」 |
| 3 | 卡死注入回归 | ✔ PASS：注入 → 回退截图 → 探针回切全链正常（角标→出图 0.31s、注入起 17.34s）。**注意**：该用例仍有 ~20-40% 概率因探针判据相位敏感而滞留截图模式（已立[方案 22](./22-恢复探针判据相位敏感修复.md)） |
| 4 | WS 瞬断回归 | ✔ PASS：6a/6b 路径不变（瞬断回退截图、指数退避重连自动回切；ws-open→出图 4.51s） |
| 5 | .18「0.4fps 故障型」复检 | 判定**不成立/被干扰**：600s 实测 3.87fps 持续出帧（突发 + ~6s 静默节律）、最大间隔仅 6.21s、0 error/0 stalled——此前「间歇冻结」观察被本缺陷主导的呼吸回退干扰（详见 5.4） |

**结论**：方案 19 验收第 1 项由 FAIL 转 PASS；呼吸循环彻底消失，stalled
回归为唯一回退源。复测同时暴露两个后续项：O6 自适应码率静止误判
（[方案 21](./21-自适应码率静止误判治理.md)）与恢复探针判据相位敏感
（[方案 22](./22-恢复探针判据相位敏感修复.md)），均已立项待审。
回写：`tests/manual/verify_stream_stability_19.md` 复测记录小节、
`方案/进度追踪.md` 第二十九次更新。
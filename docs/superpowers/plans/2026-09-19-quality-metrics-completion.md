...
- **遗留清零（2026-09-21）**：本计划遗留全部处置——建议 3 transport 死模块删除（`9f1f699`）；`tests/integration/` 归档 `docs/archive/tests-integration-2026-09/`（合法冒烟 `test_health.py` 迁入 `tests/unit/`，门禁命令移除失效 `--ignore`）；`docs/API.md`「已知问题」6 项处置（`d6e66f2`/`9c5f4b1`/`3d62933`）。处置后后端门禁 540 passed + 1 skipped、覆盖率 89.64%。详见进度追踪第二十三次更新。
...
每批变更收尾（推送前）执行一次独立审查并留档：

1. **触发时机**：一个方案或批次的所有提交完成后、推送 master 之前
2. **执行方式**：由 AI 以独立审查者身份（新会话/子代理，不携带实现过程上下文）对该批次的提交区间做代码走查
3. **审查内容**：正确性与边界（含并发、资源泄漏）、与方案文档的宣称是否一致、测试覆盖是否充分、架构与核心约束符合性
4. **留档**：产出 `docs/reviews/YYYY-MM-DD-<主题>.md`，包含：审查范围（commit 区间）、发现清单（按 阻塞/建议/观察 分级）、结论（通过 / 有条件通过 / 打回）、处置结果
5. **处置**：阻塞项必须修复后才能推送；建议项由批次负责人决定是否本批处理，未处理项在留档中记录原因
